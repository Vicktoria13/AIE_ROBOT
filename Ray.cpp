...
int Ray::rayon_unitaire(float alpha,  std::array<std::array<int, 15>, 15> maze, sf::RenderWindow* window, int taille_case){
    int distance = 0;
    int nx = this->pos_x;
    int ny = this->pos_y;
    ...
    while (Ny < 15 && Nx<15 && maze[Ny][Nx] != 1 && maze[Ny][Nx] != 2 && maze[Ny][Nx] !=3){
        ...
        // tracé du point
        point.setFillColor(sf::Color::Green); // Définit la couleur du cercle en blanc
